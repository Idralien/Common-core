...
InviteCommand::InviteCommand(void) : BaseCommand("INVITE") {}
...
void	InviteCommand::execute(void) const {
	...
	IRCUser		*target = this->_server->findUserByName(this->_args[1]);
	IRCChannel		*channel = this->_server->findChannelByName(this->_args[2]);

	if (!target)
		throw ERR_NOSUCHNICK(userId, this->_user->getNickname(), this->_args[1]);

	if (!channel) { // channel is inexistant, creating one
		IRCChannel *newChannel = new IRCChannel(this->_args[2]);
