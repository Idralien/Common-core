...
NickCommand::NickCommand(void) : BaseCommand("NICK") {}
...
void	NickCommand::execute(void) const {
	...
	if (!this->_server->isNicknameAvailable(nickname)) {
		...
		nickname.append(IRCUtilities::toString(++i));
		/* Start of debug */
		LogManager::getInstance()->queue(IRCUtilities::toString(SERVER_INFO) \
			+ "IRCUser asked for nickname '" BRED + this->_args[1] \
