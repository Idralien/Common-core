...
JoinCommand::JoinCommand(void) : BaseCommand("JOIN") {}

JoinCommand::~JoinCommand(void) {}

std::string	JoinCommand::_getStringifiedUsers(IRCChannel *channel) const {
	std::vector<IRCUser*>	vect = channel->getUsers();
	std::string	users = "";

	for (std::vector<IRCUser*>::iterator it = vect.begin(); it != vect.end(); it++) {
		IRCUser	*user = *it;
...
std::string	JoinCommand::_getChannelMode(IRCChannel *channel) const {
...
void	JoinCommand::execute(void) const {
	...
	std::deque<std::string>	channels = IRCUtilities::split(this->_args[1], ",");
	std::deque<std::string>	keys;

	if (this->_args.size() == 3)
		keys = IRCUtilities::split(this->_args[2], ",");
	else
	...
	for (std::deque<std::string>::iterator channelsIt = channels.begin(); channelsIt != channels.end(); channelsIt++) {
		...
		IRCChannel	*channel = this->_server->findChannelByName(channelName);
