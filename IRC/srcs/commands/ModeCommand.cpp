...
ModeCommand::ModeCommand(void) : BaseCommand("MODE") {}

ModeCommand::~ModeCommand(void) {}

bool	ModeCommand::_addMode(IRCChannel *channel) const {
	...
	if (modes.at(0) == 'o') {
		...
		IRCUser	*target = this->_server->findUserByName(this->_args[3]);
	...
	if (modes.at(0) == 'l') {
		...
		int	limit = IRCUtilities::stoi(limitString);
...
bool	ModeCommand::_removeMode(IRCChannel *channel) const {
	std::string	userId = USER_IDENTIFIER(this->_user->getNickname(), this->_user->getUsername());
	std::string	modes = this->_args[2];
...
void	ModeCommand::execute(void) const {
	...
	if (availableModes.find(modes[0]) == std::string::npos)
		throw ERR_UNKNOWNMODE(userId, this->_user->getNickname(), IRCUtilities::toString(modes[0]));
